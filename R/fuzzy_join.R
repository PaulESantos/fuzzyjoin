...
#' @param by Columns of each to join. Can be a character vector or named character vector.
...
#'   \code{match_fun} and \code{multi_match_fun}, this is performed on the
...
fuzzy_join <- function(x, y, by = NULL, match_fun = NULL,
  ...
  stopifnot(
    ...
    "mode must be one of the specified values" = mode %in% c("inner", "left", "right", "full", "semi", "anti")
  )

  # Check that exactly one matching function is provided
  non_nulls <- sum(c(!is.null(multi_match_fun), !is.null(match_fun), !is.null(index_match_fun)))
  if (non_nulls != 1) {
  ...
  # Helper function to restore grouping
  regroup <- function(d) {
  ...
  # Helper function to remove row names safely
  ...
  # Main logic based on matching function type
  if (!is.null(match_fun)) {
    ...
    matches <- dplyr::bind_rows(lapply(seq_along(by$x), function(i) {
      ...
      indices_x <- tibble::tibble(
        ...
        tidyr::nest() |>
        dplyr::mutate(indices = purrr::map(.data$data, "indices"))
      ...
      if (n_x == 0 || n_y == 0) {
        return(tibble::tibble(i = numeric(0), x = numeric(0), y = numeric(0)))
      }
      ...
      if (length(w) == 0) {
        return(tibble::tibble(i = i, x = numeric(0), y = numeric(0)))
      }

      x_indices_l <- indices_x$indices[w %% n_x + 1L]
      y_indices_l <- indices_y$indices[w %/% n_x + 1L]
      ...
      if (!is.null(extra_cols)) {
        ...
        extra_cols_rep <- extra_cols[extra_indices + 1L, , drop = FALSE]
    ...
    # Handle multiple column matching
    if (length(by$x) > 1) {
      accept <- matches |>
        dplyr::count(.data$x, .data$y) |>
        dplyr::ungroup() |>
        dplyr::filter(.data$n == length(by$x))
      ...
      if (ncol(matches) > 3) {
        matches <- matches |>
          dplyr::mutate(name = by$x[.data$i]) |>
          dplyr::select(-"i") |>
          tidyr::pivot_longer(cols = -c("x", "y", "name"),
                              names_to = "key",
                              values_to = "value") |>
          tidyr::unite("newname", "name", "key", sep = ".") |>
          tidyr::pivot_wider(names_from = "newname",
                             values_from = "value")
      } else {
        matches <- dplyr::distinct(matches, .data$x, .data$y)
      }
    }
    ...
    if (nrow(ux) == 0 || nrow(uy) == 0) {
      matches <- tibble::tibble(x = numeric(0), y = numeric(0))
    } else {
      pairs <- matrix(NA_real_, nrow(ux), nrow(uy))
      ix <- row(pairs)
      iy <- col(pairs)
  ...
  # Handle semi and anti joins
  if (mode == "semi") {
    if (nrow(matches) == 0) {
      return(regroup(x[integer(0), , drop = FALSE]))
    }
  ...
  # Sort matches
  matches <- dplyr::arrange(matches, .data$x, .data$y)

  # Handle column name conflicts
  common_names <- intersect(colnames(x), colnames(y))
  if (length(common_names) > 0) {
    x <- dplyr::rename_at(x, dplyr::vars(dplyr::all_of(common_names)), ~ paste0(.x, ".x"))
    y <- dplyr::rename_at(y, dplyr::vars(dplyr::all_of(common_names)), ~ paste0(.x, ".y"))
  }
  ...
  # Create final result
  ret <- dplyr::bind_cols(
    unrowname(x[matches$x, , drop = FALSE]),
    unrowname(y[matches$y, , drop = FALSE])
  )

  if (ncol(matches) > 2) {
    extra_cols <- unrowname(matches[, -(1:2), drop = FALSE])
...
#' Helper function to standardize 'by' argument
#' @param by Join specification
#' @param x First data frame
#' @param y Second data frame
#' @return List with x and y column specifications
...
# '
# ' @export
# uzzy_join <- function(x, y, by = NULL, match_fun = NULL,
#                       multi_by = NULL, multi_match_fun = NULL,
#                       index_match_fun = NULL, mode = "inner", ...) {
#  # preserve the grouping of x
#  x_groups <- dplyr::groups(x)
